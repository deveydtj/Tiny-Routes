...
| `required_tap_burst_exceeds_target` | Runtime accessibility | A production strategy exceeds its difficulty's rapid-encounter cap, two-tap burst cap, or required window margin. | High-arity switch ordering or too many rapid rotations. | Reorder authored exits or redesign the route to require fewer taps. | Yes |
| `state_change_not_visible_before_decision` | Runtime readability | An objective or road-state change occurs less than one second before the next required input window. | Stateful nodes are too close or a route change is revealed late. | Add readable travel distance before the affected switch. | Yes |
